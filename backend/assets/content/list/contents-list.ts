...
export class ContentsList implements Component<Content[]> {
    ...
    private insert(content: Content, isNew: boolean = false): ContentComponent {
    ...
    public show(contents: Content[], searchTerm: string|null): void {
        this.clear();
        this.notifications.clear();

        contents.forEach((content: Content): void => {
            const contentComponent = this.insert(content)
