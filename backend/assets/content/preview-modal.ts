export const initPreviewModal = () => {
    ...
    const previewOpenBtn = document.querySelector<HTMLButtonElement>('[data-preview-open]');

    if (!previewModal || !previewOpenBtn) {
        return;
    }

    previewOpenBtn.addEventListener('click', (): void => previewModal.classList.add('active'));
